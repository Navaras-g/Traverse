...
export function Trip() {
    ...
    useEffect(() => {
        ...
        api
            .get<Listing[]>("/listings/by-ids", { params: { ids: selectedIds.join(",") } })
    ...
    return (
        <div className="max-w-3xl mx-auto px-6 py-12">
            ...
            <div className="flex flex-wrap gap-3 mb-8">
                {listings.map((listing) => (
                    <div key={listing.id} className="flex items-center gap-2 bg-white border border-[var(--color-line)] rounded-full pl-1 pr-3 py-1">
                        ...
                        <button onClick={() => toggle(listing.id)} className="text-[var(--color-muted)] hover:text-[var(--color-ink)] text-sm ml-1">
                            ×
                        </button>
                    </div>
                ))}
            ...
            <div className="flex gap-3 mb-10">
                ...
                <button onClick={clear} className="font-sans text-sm text-[var(--color-muted)] hover:text-[var(--color-ink)] transition-colors">
                    Clear trip
                </button>
            </div>
                            ...
                            return (
                                <Reveal key={day.day_number} delayMs={i * 120}>
                                    <div className="flex gap-5 border-t border-[var(--color-line)] pt-6">
                                        <div className="font-mono text-sm text-[var(--color-muted)] pt-1 w-16 shrink-0">
                                            Day {day.day_number}
                                        </div>
